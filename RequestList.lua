...
function GBB.GetDungeons( msg, name )
  ...
  local searchTerms = {} -- NEW: Track generic search terms that made it "good"
  ...
  -- Create a list of all generic search terms (like lfm, lfg, etc.)
  local genericSearchTerms = {}
  for lang, terms in pairs(GBB.searchTagsLoc) do
    for _, term in ipairs(terms) do
      genericSearchTerms[term] = true
    end
  end
  ...
  if GBB.DB.TagsZhtw then
    for key, v in pairs( GBB.tagList ) do
      if strfind( msg:lower(), key ) then
        if v == GBB.TAGSEARCH then
          isGood = true
          table.insert(searchTerms, key) -- Track the search term
    ...
    for _, p in pairs( parts ) do
      ...
      elseif x == GBB.TAGSEARCH then
        isGood = true
        table.insert(searchTerms, p) -- Track the search term
      else
        dungeons[ x ] = true
        -- Track the tag that matched, but only if it's an instance-specific tag
  ...
  elseif isGood then
    ...
    if next( dungeons ) == nil then
      dungeons[ "MISC" ] = true
      -- Store search terms that triggered MISC classification
      matchedTags["MISC"] = searchTerms
    end
